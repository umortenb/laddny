  ...
  const authForms = () => {
    if (!showRegistration) {
      return (
        <>
          <LoginForm />            
          <div>No Account yet? <TextLink onClick={() => setShowRegistration(true)}>Register</TextLink>.</div>
        </>
      )
    } else {
      return (
        <>
          <RegistrationForm />
          <div>Have Account already? <TextLink onClick={() => setShowRegistration(false)}>Login</TextLink>.</div>
        </>
      )
    }
  }
